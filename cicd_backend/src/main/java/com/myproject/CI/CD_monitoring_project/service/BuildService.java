...
public class BuildService {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(BuildService.class);
    ...
    public BuildResponse createBuild(Build build) {
        ...
        if (build.getTriggeredBy() == null ||
                build.getTriggeredBy().isEmpty()) {
            build.setTriggeredBy("api");
        }
    ...
    public List<BuildResponse> getAllBuilds() {
        ...
        if (currentUserService.hasRole(Role.DEVELOPER)) {
            ...
            return buildRepo
                    .findByProjectIdsOrderByStartTimeDesc(ids)
                    .stream()
    ...
    public BuildResponse getBuild(Long id) {
        ...
        if (b.getProject() != null) {
            currentUserService.ensureCanReadProject(
                    b.getProject().getId());
        }
    ...
    public void deleteBuild(Long id) {
        ...
        logger.info(
                "Build deleted with ID: {}",
                id);
    }

    @Transactional(readOnly = true)
    public List<BuildResponse> getBuildsByProject(
            Long projectId
    ) {
        ...
        return buildRepo
                .findByProjectIdOrderByStartTimeDesc(projectId)
                .stream()
                .map(this::mapToResponse)
                .toList();
    }
    ...
    public void processWebhook(
        ...
        logger.info(
                "Processing webhook for project: {}, build: {}",
                dto.getProjectId(),
                dto.getBuildNumber()
        );
        ...
        try {
            ...
            // NEW
            if (savedBuild.getStatus()
                    == BuildStatus.SUCCESS) {

                deploymentService.createFromBuild(
                        dto.getProjectId(),
                        savedBuild.getId()
                );
            }
            ...
            logger.info(
                    "Webhook processed successfully for project: {}, build: {}",
                    dto.getProjectId(),
                    dto.getBuildNumber()
            );
            ...
            logger.error(
                    "Error processing webhook for project: {}",
                    dto.getProjectId(),
                    e
            );
    ...
    public double getFailureRateLastHour(
        ...
        List<Build> builds =
                buildRepo.findByProjectIdAndStartTimeAfter(
                        projectId,
                        oneHourAgo
                );
    ...
    @Transactional(readOnly = true)
    public double getSuccessRateLast7Days(
            Long projectId
    ) {
    ...
    public DashboardResponse getDashboard(
        ...
        double successRate =
                getSuccessRateLast7Days(projectId);

        double failureRate =
                getFailureRateLastHour(projectId);
